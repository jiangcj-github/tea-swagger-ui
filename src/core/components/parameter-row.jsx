...
export default class ParameterRow extends Component {
  ...
  render() {
    ...
    return (
      <>
        <Form.Item label={param.get("name")} required={required} align="middle">
          <Form.Text>
            ...
            <div style={{marginBottom: "12px"}}>
              {isOAS3 && param.get("deprecated") ? "已废弃": null}
            </div>
            <div style={{marginBottom: "12px"}}>
              参数位置：{param.get("in")}
            </div>
            { !showCommonExtensions || !commonExt.size ? null :  
              <div style={{marginBottom: "12px"}}>
            ...
            {param.get("description") && 
              <div style={{marginBottom: "12px"}}>
                描述： <Markdown source={ param.get("description") }/>
              </div>}
            {(bodyParam || !isExecute) && isDisplayParamEnum &&
              <div style={{marginBottom: "12px"}}>可选值：
                <Markdown source={
                  paramEnum.map(item => item).toArray().join(", ")
                }/>
              </div>}
            {(bodyParam || !isExecute) && paramDefaultValue !== undefined &&
              <div style={{marginBottom: "12px"}}>默认值：
                <Markdown source={paramDefaultValue}/>
              </div>}
            {(bodyParam || !isExecute) && paramExample !== undefined &&
              <div style={{marginBottom: "12px"}}>示例值：
                <Markdown source={paramExample}/>
              </div>}
            {(isFormData && !isFormDataSupported) && 
              <div style={{marginBottom: "12px"}}>错误: 当前浏览器不支持FormData</div>}
            {
              isOAS3 && param.get("examples") ? (
                <div style={{marginBottom: "12px"}}>
                  <Example
