    ...
    if (
      ...
      if ("Blob" in window) {
        ...
        if(win.navigator && win.navigator.msSaveOrOpenBlob) {
            bodyEl = <div><a href={ href } onClick={() => win.navigator.msSaveOrOpenBlob(blob, download)}>下载文件</a></div>
        } else {
            bodyEl = <div><a href={ href } download={ download }>下载文件</a></div>
        }
      } else {
        bodyEl = <pre>浏览器暂不支持下载此格式的内容.</pre>
      }
