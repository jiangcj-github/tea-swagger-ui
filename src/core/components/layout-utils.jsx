...
import { Select as TSelect, SelectMultiple, Button as TButton } from "@tencent/tea-component";
...
export const Input = (props) => {
  ...
  if(props.type == "file") {
    return <>
      ...
      {file && 
        <span style={{fontSize: "12px", verticalAlign:"middle", margin: "0 0 0 12px"}}>{file.name}</span>}
      <input 
        {...props} 
        onChange={(e) => {
          onFileChange(e);
    ...
    if(multiple) {
      return (
        <SelectMultiple 
          ...
          disabled={disabled}
          clearable={allowEmptyValue ? true : false}
          options={allowedValues.map(el => ({ value: String(item), text: String(item) }))}
          placeholder="清选择"
        />
      )
    }
